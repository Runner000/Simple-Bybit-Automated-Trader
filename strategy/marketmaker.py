...
from utils.rounding import round_step
from utils.jit_funcs import nblinspace, nbgeomspace, nbround, nbabs, nbclip
...
class MarketMaker:
    ...
    def generate_quotes(self, debug=False) -> List[Tuple[str, float, float]]:
        ...
        if debug:
            ...
            print(f"Bids: {bids}")
            print(f"Asks: {asks}")
            # df = pd.DataFrame(self.ss.bybit_klines, columns=["start", "open", "high", "low", "close", "volume", "turnover"])
            # print(df)
