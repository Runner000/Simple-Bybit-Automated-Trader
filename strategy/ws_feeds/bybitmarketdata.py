...
from exchange.bybit.get.public import BybitPublicClient
from exchange.bybit.endpoints import WsStreamLinks
from exchange.bybit.websockets.handlers.kline import BybitKlineHandler
from exchange.bybit.websockets.handlers.orderbook import BybitBBAHandler
from exchange.bybit.websockets.handlers.ticker import BybitTickerHandler
from exchange.bybit.websockets.handlers.trades import BybitTradesHandler
from exchange.bybit.websockets.handlers.liquidations import BybitLiquidationsHandler
from exchange.bybit.websockets.public import BybitPublicWs
...
class BybitMarketData:
    ...
    _topics_ = ["Orderbook", "BBA", "Trades", "Ticker", "Kline", "Liquidations"]
    ...
    def __init__(self, ss: SharedState) -> None:
        ...
        self.topic_handler_map = {
            ...
            self.ws_topics[0]: BybitBBAHandler(self.ss).process,
            # self.ws_topics[2]: BybitTradesHandler(self.ss).process,
            # self.ws_topics[3]: BybitTickerHandler(self.ss).process,
            self.ws_topics[1]: BybitKlineHandler(self.ss).simple,
            self.ws_topics[2]: BybitLiquidationsHandler(self.ss).process
        }
    ...
    async def _initialize_(self) -> None:
        ...
        """
        klines = await BybitPublicClient(self.ss).klines(self.ss.interval, 300)   #****
        # trades = await BybitPublicClient(self.ss).trades(1000)
        liquidations = await BybitPublicClient(self.ss).liquidations(1000)
        BybitKlineHandler(self.ss).initialize(klines["result"]["list"])           #****
        # BybitTradesHandler(self.ss).initialize(trades["result"]["list"])
        BybitLiquidationsHandler(self.ss).initialize(liquidations["result"]["list"])

    # async def _simple_klines_(self) -> None:
    #     """
    #     Fetches the latest klines data.
    #     """
    #     klines = await BybitPublicClient(self.ss).klines(self.ss.interval, 300)
    #     data = klines["result"]["list"].reverse()
    #     self.ss.simple_bybit_klines =  data
    #     print("simple:")
    #     print(self.ss.simple_bybit_klines)
