...
from exchange.common.localorderbook import BaseOrderBook
...
class SharedState:
    ...
    def __init__(self) -> None:
        ...
        self.bybit_klines = RingBuffer(capacity=300, dtype=(float, 7))
        self.simple = True
        self.simple_bybit_klines = []
        self.bybit_liquidations = RingBuffer(capacity=1000, dtype=(float, 4))
    ...
    def _load_settings_(self, settings: Dict, reload: bool=False) -> None:
        ...
        self.sl_l = float(settings["sl_l"])
        self.sl_s = float(settings["sl_s"])
        self.tp_l = float(settings["tp_l"])
        self.tp_s = float(settings["tp_s"])
        self.adj_sl = float(settings["adj_sl"])
        self.be = float(settings["be"])
        self.pnlCandle = int(settings["pnlCandle"])
        self.pnlPcnt = float(settings["pnlPcnt"])
        self.tradeStart = None

        self.vwma_len = int(settings["vwma_len"])
        self.fast_ema_len = int(settings["fast_ema_len"])
        self.slow_ema_len = int(settings["slow_ema_len"])
        self.candle_lookback = int(settings["candle_lookback"])
        self.median_pcnt = float(settings["median_pcnt"])
        self.atr_period = float(settings["atr_period"])
        self.bband_period = int(settings["bband_period"])
        self.stddev = float(settings["stddev"])
        self.trade_dur_min = int(settings["trade_dur_min"])
