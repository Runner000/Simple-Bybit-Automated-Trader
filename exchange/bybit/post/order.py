...
from exchange.bybit.post.client import BybitPrivatePostClient
from exchange.bybit.endpoints import PrivatePostLinks
from exchange.bybit.post.types import BybitFormats
...
class Order:
    ...
    async def order_limit_simple(self, order: Tuple[str, float, float], **kwargs) -> Union[Dict, None]:
        """
        Asynchronously places a limit order on Bybit.
        ...
        Union[Dict, None]
            The response from Bybit's API if the order is successfully placed; otherwise, None.
        """
        reduce = kwargs.get("reduceOnly", False)
        ...
        side, price, qty = self._order_to_str_(order)
        if reduce:
            payload = self.formats.create_reduce_limit(side, price, qty)
        else:
            payload = self.formats.create_limit_simple(side, price, qty, str(sl), str(tp))
        return await self._submit_(endpoint, payload)
    ...
    async def amend(self, order: Tuple[str, float, float], **kwargs) -> Union[Dict, None]:
        """
        ...
        if simple:
            payload = self.formats.create_simple_amend(order_id, price)
        else: 
            payload = self.formats.create_amend(order_id, price, qty)
        return await self._submit_(endpoint, payload)

    async def set_sl_tp(self, sl: float) -> Union[Dict, None]:

        endpoint = self.endpoints.SET_SL_TP
        sl = str(sl)
        payload = self.formats.create_sl_tp(sl)
        return await self._submit_(endpoint, payload)
